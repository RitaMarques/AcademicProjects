...
df_insurance['Coherence_Premiums']=df_insurance.apply(lambda x: 1
if (summ(x.Motor, x.Household, x.Life, x.Health, x.Work_Compensation)>(x.Monthly_Salary*12)) else 0, axis=1)
df_insurance = df_insurance[df_insurance.Coherence_Premiums != 1]

del df_insurance['Birthday']

del df_insurance['Coherence_Birthday'], df_insurance['Coherence_First1'], \
    df_insurance['Coherence_First2'], df_insurance['Coherence_Salary'], \
    df_insurance['Coherence_Education'], df_insurance['Coherence_Premiums']
...
sb.heatmap(correlacoes, mask=mask, cmap=cmap, center=0, square=True, ax=ax, linewidths=.5, annot=annot1,
           fmt="s", vmin=-1, vmax=1, cbar_kws=dict(ticks=[-1,0,1]))
...
trained_model = clf.fit(complete.loc[:,['Motor','Health','Household','Life','Work_Compensation']],
                        complete.loc[:,'Education'])

imputed_values = trained_model.predict(incomplete.drop(columns=['Cust_ID','Education','First_Year',
                                                                'Monthly_Salary','Area','Children',
                                                                'CMV','Claims_Rate']))
...
incomplete.columns=['Cust_ID','First_Year', 'Monthly_Salary', 'Area', 'Children', 'CMV',
                    'Claims_Rate', 'Motor', 'Household','Health', 'Life', 'Work_Compensation', 'Education']
...
y,x = dmatrices('Monthly_Salary ~ Education + Motor + Household + Children + Life + Work_Compensation',
                data = df_insurance, NA_action='drop', return_type='dataframe')
mod = sm.OLS(y,x)
res = mod.fit()
print(res.summary())
...
#treinar os dados
neigh=regressor.fit(data_to_reg_complete.loc[:,['Education', 'Motor', 'Household', 'Children', 'Life',
                                                'Work_Compensation']], data_to_reg_complete.loc[:,['Monthly_Salary']])

#criar array com os valores previstos
imputed_Salary=neigh.predict(data_to_reg_incomplete.drop(columns=['Monthly_Salary', 'Area', 'CMV',
                                                                  'Claims_Rate', 'Cust_ID', 'First_Year', 'Health']))
...
data_to_reg_incomplete.columns=['Area', 'CMV', 'Children', 'Claims_Rate', 'Cust_ID', 'Education', 'First_Year',
                                'Health', 'Household', 'Life', 'Motor', 'Work_Compensation', 'Monthly_Salary']
...
data_to_reg_incomplete = data_to_reg_incomplete[['Area', 'CMV', 'Children', 'Claims_Rate', 'Cust_ID',
                                                 'Education', 'First_Year', 'Health', 'Household', 'Life',
                                                 'Monthly_Salary', 'Motor', 'Work_Compensation']]
...
trained_model = clf.fit(complete.loc[:,['Monthly_Salary','Motor','Household','Health',
                                        'Life','Work_Compensation']], complete.loc[:,'Children'])

imputed_values = trained_model.predict(incomplete.drop(columns=['Cust_ID','First_Year',
                                                                'Education','Area','Children','CMV','Claims_Rate']))
...
incomplete.columns=['Area','CMV','Claims_Rate','Cust_ID','Education','First_Year',
                    'Health','Household','Life','Monthly_Salary', 'Motor','Work_Compensation', 'Children']
...
def new_variables(df_insurance):
    ...
    df_insurance['Total_Premiums']=df_insurance.loc[:,['Motor','Household','Health','Life',
                                                       'Work_Compensation']][df_insurance>0].sum(1)
...
cmap = sb.diverging_palette(49, 163, as_cmap=True)
sb.heatmap(correlacoes, mask=mask, cmap=cmap, center=0, square=True, ax=ax, linewidths=.5,
           annot=annot1, fmt="s", vmin=-1, vmax=1, cbar_kws=dict(ticks=[-1,0,1]))
sb.set(font_scale=1)
sb.set_style('white')
bottom, top = ax.get_ylim()             
ax.set_ylim(bottom + 0.5, top - 0.5)
